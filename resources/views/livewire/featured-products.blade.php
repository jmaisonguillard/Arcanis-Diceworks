...
<div class="w-full flex flex-col justify-center items-center px-64">
    <div class="flex flex-row border-b border-athens py-16 justify-between w-full">
        ...
        <a href="{{ route('products') }}"
           class="rounded-full bg-bunker text-white flex flex-row justify-center items-center py-2 px-4">
            {{ __('See All Products') }}
                    ...
                    <div class="relative">
                        @if($product->created_at >= Carbon::now()->subMonth())
                            <div class="absolute flex flex-row top-4 left-4 right-4">
                                <div
                                    class="bg-dodger-blue bg-opacity-10 text-dodger-blue text-xs font-bold py-2 px-6 rounded">
                                    New In
                                </div>
                            </div>
                        @endif
                        ...
                        @if(!$product->stock_quantity)
                            <div class="absolute flex flex-row top-4 left-4 right-4">
                                <div class="bg-minsk bg-opacity-10 text-minsk text-xs font-bold py-2 px-6 rounded">Sold
                                    Out
                                </div>
                            </div>
                        @endif
                        <a href="{{ route('products.show', [$product->slug]) }}" class="absolute top-4 right-4">
                            <div class="bg-heliotrope bg-opacity-10 text-heliotrope text-xs font-bold p-2 rounded">
                                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"
                                     stroke-width="1.5" stroke="currentColor" class="w-[13px] h-[13px]">
                        ...
                        <div
                            class="absolute bottom-0 right-0 bg-bunker text-white font-semibold text-2xl w-[76px] h-[76px] flex justify-center items-center align-middle">
                            @php
                            ...
                            <span>
                                ...
                                @if($decimal > 0)
                                    <span class="absolute top-[6px] right-[6px] text-sm">{{ $decimal }}</span>
                                    <!-- Display decimal part smaller -->
                                @endif
                            </span>
                        </div>
